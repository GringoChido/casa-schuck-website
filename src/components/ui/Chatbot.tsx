      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            {/* WhatsApp Fallback */}
            <div className="px-4 py-2 bg-surface-container-low border-t border-outline-variant/15 flex-shrink-0">
              <a
                href={whatsappUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="text-xs text-secondary hover:text-on-secondary-container transition-colors flex items-center gap-1.5"
              >
            ...
            {/* Input Area */}
            <div className="px-4 py-3 border-t border-outline-variant/15 flex gap-2 flex-shrink-0 bg-white">
              <input
                type="text"
                value={input}
                ...
                className="flex-1 bg-surface-container-low border border-outline-variant/20 rounded-full px-4 py-2.5 text-sm
                           text-on-surface placeholder:text-on-surface/40 focus:outline-none
                           focus:border-secondary/50 transition-colors"
              />
              <button
                onClick={handleSend}
                disabled={!input.trim()}
                className="w-10 h-10 bg-black hover:bg-secondary disabled:bg-outline-variant
